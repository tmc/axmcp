...
func PlatformStatus() PlatformReport {
	caps := []PlatformCapability{
		...
		{Name: "input", Available: true, Message: "pixel clicks, drags, key presses, and text use background Win32 messages; foreground clicks use opt-in SendInput; UIA invoke/toggle/select/expand-collapse/set-value patterns are available"},
