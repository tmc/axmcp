...
func PlatformStatus() PlatformReport {
	...
	default:
		caps = append(caps, PlatformCapability{Name: "input", Available: true, Message: "xdotool-backed X11 window-targeted input and opt-in foreground clicks are available at " + xdotoolPath})
	}
	if display == "" {
	...
	return PlatformReport{
		...
		Capabilities: caps,
		Message:      "Linux native desktop automation has X11 state, screenshots, window-targeted xdotool input, a bounded AT-SPI reader, AT-SPI element action dispatch, and limited AT-SPI/xdotool set-value support; Wayland support is not implemented",
	}
}
